...
def _revert_ec2(instance_id: str, baseline: Dict, region: str) -> Dict:
    ...
    # Revert state
    if baseline.get('state') == 'running':
        ec2.start_instances(InstanceIds=[instance_id])
        actions.append("Started instance")
    elif baseline.get('state') == 'stopped':
        ec2.stop_instances(InstanceIds=[instance_id])
        actions.append("Stopped instance")
    ...
    return {
        "status": "success",
        "message": f"✅ Reverted EC2 {instance_id}: {', '.join(actions)}",
        "actions": actions
    }
...
def _revert_security_group(sg_id: str, baseline: Dict, region: str) -> Dict:
    ...
    if baseline.get('ingress_rules'):
        ec2.authorize_security_group_ingress(
            GroupId=sg_id,
            IpPermissions=baseline['ingress_rules']
        )
...
def _terminate_lambda(function_name: str, region: str, force: bool) -> Dict:
    ...
    return {
        "status": "deleted",
        "message": f"❌ Deleted Lambda function {function_name}"
    }
...
def _terminate_dynamodb(table_name: str, region: str, force: bool) -> Dict:
    ...
    return {
        "status": "deleting",
        "message": f"❌ Deleting DynamoDB table {table_name}"
    }
...
def _terminate_sqs(queue_url: str, region: str, force: bool) -> Dict:
    ...
    return {
        "status": "deleted",
        "message": f"❌ Deleted SQS queue {queue_url}"
    }
