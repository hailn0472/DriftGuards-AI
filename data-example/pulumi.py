...
_plan: Dict[str, Any] = {
    "meta": {
        ...
        "message": "Pulumi SDK available" if PULUMI_AVAILABLE else "Pulumi SDK not available; producing dry-run plan",
    },
    "resources": []
}
...
if not PULUMI_AVAILABLE:
    ...
    # VPC
    record_resource('awsx.ec2.Vpc', 'ecom-vpc', {
        'cidr_block': '10.0.0.0/16',
        'number_of_availability_zones': 2
    })

    # EKS
    record_resource('eks.Cluster', 'frontend-eks', {
    ...
    record_resource('aws.iam.Role', 'ecsTaskExecutionRole', { 'assume_role_policy': 'sts:AssumeRole for ecs-tasks.amazonaws.com' })
    record_resource('aws.cloudwatch.LogGroup', 'ecs-log-group', { 'retention_in_days': 14 })
    record_resource('aws.ecs.TaskDefinition', 'order-task', {
        ...
        'containers': [
            { 'name': 'order-service', 'image': 'amazon/amazon-ecs-sample', 'portMappings': [{'containerPort': 8080}] },
            { 'name': 'instana-agent', 'image': 'instana/agent:latest', 'env': ['INSTANA_AGENT_KEY', 'INSTANA_ENDPOINT'] }
        ]
    })

    # Aurora
    record_resource('aws.rds.SubnetGroup', 'aurora-subnet-group', { 'subnet_ids': ['private-subnet-1', 'private-subnet-2'] })
    record_resource('aws.rds.Cluster', 'aurora-cluster', { 'engine': 'aurora-mysql', 'engine_mode': 'provisioned' })
    record_resource('aws.rds.ClusterInstance', 'aurora-instance-1', { 'instance_class': 'db.r5.large' })

    # DynamoDB
    record_resource('aws.dynamodb.Table', 'session-table', { 'attributes': [{'name': 'sessionId', 'type': 'S'}], 'billing_mode': 'PAY_PER_REQUEST' })
    ...
    record_resource('aws.msk.Cluster', 'msk-cluster', { 'kafka_version': '2.8.1', 'number_of_broker_nodes': 3 })

    # Instana on EKS — DaemonSet
    record_resource('k8s.core.v1.Namespace', 'instana', {})
    record_resource('k8s.apps.v1.DaemonSet', 'instana-agent', { 'image': 'instana/agent:latest', 'mounts': ['/proc','/sys'] })

    # ROSA placeholder
    record_resource('note', 'rosa-placeholder', { 'instructions': 'Use rosa CLI to provision ROSA cluster and apply daemonset with kubeconfig' })
    ...
    try:
        ...
        print('Open the JSON file to inspect the intended resources. To run for real, install Pulumi and providers:')
...
if PULUMI_AVAILABLE:
    ...
    execution_role = aws.iam.Role('ecsTaskExecutionRole', assume_role_policy=json.dumps({
        'Version': '2012-10-17',
        'Statement': [{
    ...
    aws.iam.RolePolicyAttachment('ecsExecPolicyAttach', role=execution_role.name,
    ...
    task_def = aws.ecs.TaskDefinition('order-task', family='order-task-family', cpu='512', memory='1024', network_mode='awsvpc', requires_compatibilities=['FARGATE'], execution_role_arn=execution_role.arn, container_definitions=pulumi.Output.all(instana_agent_key).apply(lambda args: json.dumps([
        {
            'name': 'order-service',
    ...
    aurora_cluster = aws.rds.Cluster('aurora-cluster', engine='aurora-mysql', engine_mode='provisioned', master_username='admin', master_password=config.require_secret('db:password'), db_subnet_group_name=subnet_group.id)

    # at least one instance
    aurora_instance = aws.rds.ClusterInstance('aurora-instance-1', cluster_identifier=aurora_cluster.id, instance_class='db.r5.large')

    # 5) DynamoDB for session storage
    db_table = aws.dynamodb.Table('session-table', attributes=[{'name': 'sessionId', 'type': 'S'}], hash_key='sessionId', billing_mode='PAY_PER_REQUEST')
    ...
    msk_cluster = aws.msk.Cluster('msk-cluster', cluster_name='ecom-msk', kafka_version='2.8.1', number_of_broker_nodes=3, broker_node_group_info={
    ...
    instana_namespace = k8s.core.v1.Namespace('instana', metadata={'name': 'instana'}, opts=pulumi.ResourceOptions(provider=k8s_provider))
                    ...
                    'containers': [
                        {
                            ...
                            'image': 'instana/agent:latest',
                            'env': [
                                {'name': 'INSTANA_AGENT_KEY', 'valueFrom': {'secretKeyRef': {'name': 'instana-agent-secret', 'key': 'agentKey'}}},
                                {'name': 'INSTANA_ENDPOINT', 'value': instana_endpoint}
                            ],
    ...
    instana_secret = k8s.core.v1.Secret('instana-agent-secret', metadata={'namespace': instana_namespace.metadata['name']}, string_data={'agentKey': instana_agent_key.apply(lambda k: k if isinstance(k, str) else str(k))}, opts=pulumi.ResourceOptions(provider=k8s_provider))

    instana_daemonset = k8s.apps.v1.DaemonSet('instana-agent-ds', metadata={'namespace': instana_namespace.metadata['name']}, spec=instana_daemonset_manifest['spec'], opts=pulumi.ResourceOptions(provider=k8s_provider))
    ...
    print('This module will not attempt to create cloud resources. To run for real, install Pulumi and the required providers:')
    print('  pip install pulumi pulumi-aws pulumi-awsx pulumi-eks pulumi-kubernetes')
